    ...
    return tf.tidy(() => {
      ...
      const inputTensors = range(this.batchSize, 0, 1).map(batchIdx => {
        ...
        if (input instanceof tf.Tensor) {
          let imgTensor = isTensor4D(input) ? input : tf.expandDims(input) as tf.Tensor4D
          imgTensor = padToSquare(imgTensor, isCenterInputs)

          if (imgTensor.shape[1] !== inputSize || imgTensor.shape[2] !== inputSize) {
            imgTensor = tf.image.resizeBilinear(imgTensor, [inputSize, inputSize]) as tf.Tensor4D
          }
        ...
        if (input instanceof env.getEnv().Canvas) {
          return tf.browser.fromPixels(imageToSquare(input, inputSize, isCenterInputs)) as tf.Tensor3D
        }

        throw new Error(`toBatchTensor - at batchIdx ${batchIdx}, expected input to be instanceof tf.Tensor or instanceof HTMLCanvasElement, instead have ${input}`)
      })

      const batchTensor = tf.reshape(tf.stack(inputTensors.map(t => tf.cast(t, 'float32'))), [this.batchSize, inputSize, inputSize, 3]) as tf.Tensor4D
